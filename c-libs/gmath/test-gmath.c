...
{
	printf("\nQuaternion Tests\n");
	g_object *test_object = calloc(1, sizeof(*test_object));
	//test_object->rot = (quaternion){ (float)1/3, (float)1/3, (float)1/3, 1};
	test_object->localpos = (vec3f){ 2, 2, 2 };
	set_quaternion_g_object(test_object, 90, 0.25, 0.3, 0.3);
	printf("\nTest_object local position = ");
	printvec3f(test_object->localpos);
	printf("Test_object global position = ");
	printvec3f(test_object->globalpos);
	printf("Test_object rotation (x, y, z, w) = ");
	printvec4f(test_object->rot);
	free(test_object);
}
